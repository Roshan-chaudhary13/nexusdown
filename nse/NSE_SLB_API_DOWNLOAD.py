...
NSECMPath=json_data["config"]["Path"]["SLB"]["NSESLB"]
...
NSE_SLB_Config = json_data["config"].get("Master_Files", {}).get("NSE_SLB", {})
...
SLB_Mode = json_data["config"].get("Extraction_Modes", {}).get("SLB", "Custom")

# Pre-compute resolved target regexes
target_files = []
if SLB_Mode == "Custom" and NSE_SLB_Config:
    for key, val in NSE_SLB_Config.items():
...
MemberPathFolder=json_data["config"]["API_DETAILS"]["NSE_API"].get("Path_SLB_Member", ["Onlinebackup", "Reports", "Reports/Standard report", "/Reports/Dnld/PNL01"])
CommonPathFolder=json_data["config"]["API_DETAILS"]["NSE_API"].get("Path_SLB_Common", ["Bhavcopy", "", "Contracts", "MarketReports", "Marketreports"])
...
logging.basicConfig(filename=workpath+'\logs\\NSE_SLB_API_DOWNLOAD_'+str(FromDate)+'.log', level=logging.INFO, format='[ %(asctime)s] - [%(levelname)s ] - %(message)s')
...
if Login_status == "success":
    ...
    def MemberFiles():
        for MemberFolderName in MemberPathFolder:
            ...
            #             try:
            #                 with open(str(NSECMPath + "\\" + FromDate + '\\temp\\') + str(FileCEP),'wb') as out_file:
                        ...
                        if FileType == "File":
                            ...
                            if is_target:
                                ...
                                if not FileExist and not FileExistTemp:
                                    ...
                                    if File_Down_Resp!='error':
                                        try:
                                            file_path = str(NSECMPath + "\\" + FromDate + '\\temp\\') + str(CommonFileName)
                                            with open(file_path, 'wb') as out_file:
                                                shutil.copyfileobj(File_Down_Resp, out_file)
                                                print(CommonFileName+" Downloading...")
                                                logging.info(CommonFileName+" Downloading...")
                                            auto_extract_file(file_path, NSECMPath + "\\" + FromDate)
                                        except Exception as e:
                                            logging.error(str(dt.now())+ " ------- NSECM API -----"+str(e))
                                            print(e)
                                    else:
