...
NSEFOPath=json_data["config"]["Path"]["FO"]["NSEFO"]
...
FO_NSE_Config = json_data["config"].get("Master_Files", {}).get("FO_NSE", {})
...
FO_Mode = json_data["config"].get("Extraction_Modes", {}).get("FO", "Custom")

# Pre-compute resolved target regexes
target_files = []
if FO_Mode == "Custom" and FO_NSE_Config:
    for key, val in FO_NSE_Config.items():
...
def is_target_file(server_filename, targets):
    # Always allow contract.gz for FO segment
    if server_filename.lower() == "contract.gz" or server_filename.lower() == "contract":
        return True
    if not targets:
...
MemberPathFolder=json_data["config"]["API_DETAILS"]["NSE_API"]["Path_FO_Member"]
CommonPathFolder=json_data["config"]["API_DETAILS"]["NSE_API"]["Path_FO_Common"]
...
pathtemp = (NSEFOPath)+str(FromDate)+ '\\temp'

tempPath=os.path.exists((NSEFOPath)+str(FromDate)+ '\\temp')
datePath=os.path.exists((NSEFOPath)+str(FromDate))

# logging.basicConfig(filename=workpath + "\logs\script_logs_" + str(dt.today().strftime("%d%m%Y")) + ".log")
#logging.basicConfig(filename=workpath+'\logs\\NSE_FO_API_DOWNLOAD_'+FromDate+'.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logging.basicConfig(filename=workpath+'\logs\\NSE_FO_API_DOWNLOAD_'+FromDate+'.log', level=logging.INFO, format='[ %(asctime)s] - [%(levelname)s ] - %(message)s')

if not datePath==True:
    os.makedirs(NSEFOPath+FromDate, exist_ok=True)
...
def unzip_gz_file(source_path, dest_path):
    ...
    auto_extract_file(source_path, os.path.dirname(source_path))
...
if Login_status == "success":
            ...
            if isinstance(FileNameList, list):
                def download_task_member(i):
                    ...
                    if FileType == "File":
                        ...
                        if is_target:
                            ...
                            dest_dir = os.path.join(NSEFOPath, FromDate, subfolder)
                            ...
                            if not FileExist and not FileExistTemp:
                                File_Down_Resp = Class_Object.File_Download(Login_token, Member_File_Download_Url, Segment,
                                                                            MemberFolderName, MemberFileName)
                                if File_Down_Resp!='error':
                                    ...
                                    print(MemberFileName + " Error in files")
                                    logging.error(MemberFileName + " Error in files")
                ...
                try:
                    with ThreadPoolExecutor(max_workers=5) as executor:
                        executor.map(download_task_member, FileNameList)
                except Exception as e:
                    logging.error(" ------- NSEFO API -----" + str(e))
                    print(e)
    #*********************COMMON FILES****************
    def CommonFiles():
        ...
        for CommonFolderName in CommonPathFolder:
            ...
            if isinstance(FileNameList, list):
                def download_task_common(i):
                    CommonFileName = i['name']
                    FileType = i["type"]
                    if FileType == "File":
                        is_target = is_target_file(CommonFileName, target_files) if target_files else (FromDate in CommonFileName or CommonFileName=='contract.gz' or RevFromDate in CommonFileName)
                        if is_target:
                            # Save directly to category subfolder
                            ...
                            if not FileExist and not FileExistTemp:
                                File_Down_Resp = Class_Object.File_Download(Login_token, Common_File_Download_Url,
                                                                            Segment,
                ...
                try:
                    with ThreadPoolExecutor(max_workers=5) as executor:
                        executor.map(download_task_common, FileNameList)
                except Exception as e:
                    logging.error(str(dt.now()) + " ------- NSEFO API -----" + str(e))
                    print(e)
    ...
    print(Login_status)
    logging.error(" ------- NSEFO API -----"+str(Login_status))


try:
    try:
        gz_file = os.path.join(NSEFOPath, FromDate, 'Contracts', 'contract.gz')
        dest_path = os.path.join(NSEFOPath, FromDate, 'Contracts', 'contract')
        unzip_gz_file(gz_file,dest_path)   
    except Exception as e:
        print(e)
        logging.error(" ------- NSEFO API -----" + str(e))
        
    try:
        pathtemp = os.path.join(NSEFOPath, FromDate, 'temp')
        os.makedirs(pathtemp, exist_ok=True)
...
local_files = set()
for root, dirs, files in os.walk(os.path.join(NSEFOPath, FromDate)):
    for f in files:
        name = f.lower()
        if name.endswith(".gz"):
            name = name[:-3]
